...
While Not EOF(NAR)
    cona = cona + 1
    Get #NAR, cona, argra
    If (argra.num_pro = Val(MENUPROFE.LBLNumProfe.Caption)) And RTrim(argra.grado) = RE22 And RTrim(argra.nom_grup) = Combo2.Text And argra.num_area = que Then
        NAR = FreeFile
...
If (lw <> 5) Then
    ...
    If VeriPeriodo(lw, "periodosD") = False Then
        Command5.Enabled = False
         MsgBox "EL PERIODO " & Combo1 & " SOLO ESTA DISPONIBLE PARA CONSULTA", 32, "Grabar desempeños"
    Else
...
salbla:
    ...
    ' ****Se verifica si está activa la opción de cerrar plantilla****
    If Dir(Ruta & Combo2.Text & que & lw & ".fnp") = "" Then
        Check2.Value = 0
    Else
        Check2.Value = 1
    End If
...
If RESP = vbYes Then
    ...
    ' ****Guardamos el archivo de verificación de planillas cerradas****
    If Check2.Value = 1 Then
        If Dir(Ruta & Label6.Caption & ".fnp") = "" Then
            Open Ruta & Label6.Caption & ".fnp" For Output As #NAR
            Write #NAR, "1"
            Close #NAR
        End If
    Else
        If Dir(Ruta & Label6.Caption & ".fnp") <> "" Then
            Kill Ruta & Label6.Caption & ".fnp"
        End If
    End If
...
If RESP = vbYes Then
   ...
   For I = 1 To (Mt_desemp.Rows - 1)
      ...
      For J = 3 To Mt_desemp.Cols - 1
        ...
        Mt_desemp.Row = I
        Mt_desemp.Col = J
        If Mt_desemp.CellForeColor = RGB(255, 0, 0) Then
