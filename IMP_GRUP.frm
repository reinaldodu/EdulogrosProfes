...
If Dir(Ruta & Text1.Text & ".gru") = "" Then
    MsgBox "GRUPO NO LE CORRESPONDE", 48, "IMPRIMIR GRUPO"
    Text1.SetFocus
    Exit Sub
End If
...
Get #NAR, dire, profe
Close #NAR
Printer.Print "DIRECTOR: " & RTrim(profe.nombres) & " " & RTrim(profe.apellidos)
Printer.CurrentY = 1.5
Printer.CurrentX = 6
Printer.Print "ALUMNOS JORNADA " & YUS & " GRUPO " & Text1.Text
Printer.CurrentY = 3
Printer.CurrentX = 0.5
...
If (Dir(Ruta & "areagra.edu") <> "") Then
    ...
    While Not EOF(NAR)
        ...
        If argra.num_pro = Val(MENUPROFE.LBLNumProfe.Caption) Then
            VALI2 = False
            For I = 0 To (MATI4.Rows - 1)
                If MATI4.TextMatrix(I, 0) = RTrim(argra.nom_grup) Then
                    VALI2 = True
                    Exit For
                End If
            Next I
            If VALI2 = False Then
                MATI4.Rows = plo + 1
                MATI4.TextMatrix(plo, 0) = RTrim(argra.nom_grup)
                plo = plo + 1
            End If
        End If
    Wend
