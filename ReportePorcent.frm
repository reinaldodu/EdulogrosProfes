...
While Not EOF(NAR)
    cona = cona + 1
    Get #NAR, cona, argra
    If (argra.num_pro = Val(MENUPROFE.LBLNumProfe.Caption)) And RTrim(argra.grado) = RE22 And RTrim(argra.nom_grup) = Combo1.Text And argra.num_area = que Then
        NAR = FreeFile
...
If Check1.Value = 1 Then
    ' Mostrar porcentajes por periodo
    For h = 1 To 4
        If Mtx_Reporte.TextMatrix(0, h + 1) = "" Then
            Cont_Lgr = 0
            FERT = 0
            If Dir(Ruta & Left(Combo1.Text, 1) & Left(RE22, 3) & que & h & ".lgr") <> "" Then
                Open Ruta & Left(Combo1.Text, 1) & Left(RE22, 3) & que & h & ".lgr" For Random As #NAR Len = Len(logru)
                While Not EOF(NAR)
                    FERT = FERT + 1
                    Get #NAR, FERT, logru
                    If Trim(logru.indicador) = "L" Then
                ...
                Close #NAR
            End If
            If Dir(Ruta & Left(Combo1.Text, 1) & Left(RE22, 3) & que & h & ".ptj") <> "" Then
                Cont_Porc_Lgr = 0
                For t = 1 To Cont_Lgr
                    Open Ruta & Left(Combo1.Text, 1) & Left(RE22, 3) & que & h & ".ptj" For Random As #NAR Len = Len(porcent_manual)
                    Get #NAR, t, porcent_manual
                    Close #NAR
                    Cont_Porc_Lgr = Cont_Porc_Lgr + porcent_manual.porcent_logro
                Next t
                Mtx_Reporte.Row = 0
                Mtx_Reporte.Col = h + 1
                Mtx_Reporte.CellForeColor = RGB(255, 0, 0)
                Mtx_Reporte.Text = Cont_Porc_Lgr & "% (pron)"
            Else
                Exit For
            End If
        End If
    Next h
    ' Recorrer notas y pronosticar próxima nota
    For J = 2 To Mtx_Reporte.Rows - 2
        AcumulaNota = 0
        PorcentAcumulado = 0
        For h = 2 To 5
            AcumulaNota = AcumulaNota + Val(Mtx_Reporte.TextMatrix(J, h)) * Val(Mtx_Reporte.TextMatrix(0, h)) / 100
            PorcentAcumulado = PorcentAcumulado + Val(Mtx_Reporte.TextMatrix(0, h))
            Minimo = 0.7 * PorcentAcumulado
            If Mtx_Reporte.TextMatrix(J, h) = "" Then
                VrPronostico = Format((Minimo - AcumulaNota) / (Val(Mtx_Reporte.TextMatrix(0, h)) / 100), "#.00")
                Mtx_Reporte.Row = J
                Mtx_Reporte.Col = h
                Mtx_Reporte.CellForeColor = RGB(255, 0, 0)
                If VrPronostico > 70 Then
                    Mtx_Reporte.CellFontBold = True
                Else
                    Mtx_Reporte.CellFontBold = False
                End If
                Mtx_Reporte.Text = VrPronostico
                Exit For
            End If
        Next h
    Next J
...
If RESP = vbYes Then
   ...
   If Check1.Value = 1 Then
        Printer.CurrentX = 1
        Printer.Print "(Este reporte incluye pronóstico de notas)"
   End If
   Printer.CurrentY = 3
