...
Begin VB.Form COPEGA 
   ...
   Caption         =   "Grabar Logros y Observaciones por grado"
   ClientHeight    =   6735
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   9510
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Frame Frame3 
         ...
         Begin VB.ComboBox Combo2 
            Height          =   315
            ItemData        =   "COPEGA.frx":0487
            Left            =   960
            List            =   "COPEGA.frx":0497
            TabIndex        =   1
            Text            =   "UNICA"
            Top             =   240
            Width           =   1215
      ...
      Begin VB.Frame Frame2 
         ...
         Begin VB.ComboBox Combo5 
            Height          =   315
            ItemData        =   "COPEGA.frx":04B8
            Left            =   600
            List            =   "COPEGA.frx":04BF
            TabIndex        =   6
            Text            =   "L"
            Top             =   240
            Width           =   735
...
While Not EOF(NAR)
    cona = cona + 1
    Get #NAR, cona, argra
    If (argra.num_pro = Val(MENUPROFE.LBLNumProfe.Caption)) And (RTrim(argra.grado) = RTrim(Combo3.Text) And (argra.num_area = NA)) Then
        Close #NAR
        GoTo intel
    End If
Wend
...
If (Dir(Ruta & "materia.edu") <> "") And (Dir(Ruta & "areagra.edu") <> "") Then
    ...
    While Not EOF(NAR)
        ...
        If argra.num_pro = Val(MENUPROFE.LBLNumProfe.Caption) Then
            ...
            For I = 0 To (Combo3.ListCount - 1)
                If Combo3.List(I) = RTrim(argra.grado) Then
                    VALI2 = True
                    Exit For
                End If
            Next I
            If VALI2 = False Then
                Combo3.AddItem RTrim(argra.grado)
            End If
            NAR = FreeFile
            Open Ruta & "materia.edu" For Random As #NAR Len = Len(mate)
            ...
            For I = 0 To (Combo4.ListCount - 1)
                If Combo4.List(I) = RTrim(mate.nom) Then
                    VALI2 = True
                    Exit For
                End If
            Next I
    ...
    Combo3.Text = Combo3.List(0)
    Combo4.Text = Combo4.List(0)
    If (RTrim(Combo3.Text) = "") Or (RTrim(Combo4.Text) = "") Then
